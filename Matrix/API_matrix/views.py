...
class MatrixVectorView(APIView):
    ...
    @extend_schema(
        ...
        examples=[OpenApiExample('2x2 matrix',
                                 value=MatrixSerializer(
                                     instance=MatrixVector(2, [[1, 2], [2, 1]], [[2, 4], [4, 2]])).data,
                                 response_only=True,
                                 status_codes=[200]
                                 ),
    ...
    def get(self, request):
        ...
        if a is None:
            return Response('Умножаемое число не введено', 400)

        try:
            mulMatrix = int(a) * matrix
        ...
        matrixVector = MatrixVector(size, matrix, mulMatrix)
